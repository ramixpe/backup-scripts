...
for items in open('hosts.txt').read().strip().splitlines():
    file_name = ('IP_' + items.split('.')[-1] + '_' + date +'.txt')
    try:
        with open(file_name, 'w') as f:
            connect = ConnectHandler(host=items, username='xr', password = 'xr', device_type= 'cisco_xr')
            router_configuration = connect.send_command('show run')
            f.write(router_configuration)
    except NetMikoTimeoutException:
        print(f'the router {ip} is offline')
